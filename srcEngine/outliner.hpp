...
public:
	...
	static void generateOutlinedShape(const std::vector<glm::vec3> vertices, float width, std::vector<glm::vec3> &vertOutline, std::vector<mgeng::triIx16> &triOutline, std::vector<glm::vec3> &vertFill, std::vector<mgeng::triIx16> &triFill) {
...
// === create outline polygons ===
		std::vector<GLushort> vOuter;
		std::vector<GLushort> vInner;
		for (unsigned int ix = 0; ix < nVertices; ++ix) {
			GLushort ixOuter = vertOutline.size();
			vertOutline.push_back(vertices[ix]);
			vOuter.push_back(ixOuter);

			GLushort ixInner = vertOutline.size();
			vertOutline.push_back(kneeList[ix]);
			vInner.push_back(ixInner);
		}
		for (unsigned int ix = 0; ix < nVertices; ++ix) {
			int ix2 = (ix + 1) % nVertices;
			triOutline.push_back( { vOuter[ix], vOuter[ix2], vInner[ix2] });
			triOutline.push_back( { vOuter[ix], vInner[ix2], vInner[ix] });
		}
...
// === render triangle ===
			triFill.push_back( { vList[ixPrev].id, vList[ixMax].id, vList[ixNext].id });
